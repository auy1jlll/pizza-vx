...
async function restoreAllData(backupFile) {
  ...
  try {
    ...
    // Clean user data for production schema
    if (backup.users?.length) {
      const cleanUsers = backup.users.map(user => {
        const { customerProfile, employeeProfile, customerAddresses, customerFavorites, ...cleanUser } = user;
        return cleanUser;
      });
      
      await prisma.user.createMany({ data: cleanUsers });
      console.log(`✅ Restored ${cleanUsers.length} users`);
    }

    // Restore employee profiles separately
    ...
    // Restore customer profiles separately
    ...
    if (backup.menuItems?.length) {
      // Clean menu items data for production schema
      const cleanMenuItems = backup.menuItems.map(item => {
        const { category, customizationGroups, ...cleanItem } = item;
        return cleanItem;
      });
      
      await prisma.menuItem.createMany({ data: cleanMenuItems });
      console.log(`✅ Restored ${cleanMenuItems.length} menu items`);
    }
...
// Usage: node restore-data-production.js path/to/backup.json
if (require.main === module) {
  const backupFile = process.argv[2];
  if (!backupFile) {
    console.error('Usage: node restore-data-production.js <backup-file>');
    process.exit(1);
  }
