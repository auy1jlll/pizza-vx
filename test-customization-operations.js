...
async function testCustomizationGroupOperations() {
  try {
    console.log('🔍 Testing customization group operations...\n');
    ...
    // Test GET all groups
    console.log('\n📡 Testing GET /api/admin/customization-groups...');
    const getGroupsResponse = await fetch('http://localhost:3005/api/admin/customization-groups', {
      headers: {
        'Cookie': cookies || ''
      }
    });
    ...
    if (getGroupsResponse.ok) {
      ...
      if (groups.length > 0) {
        ...
        const getSingleResponse = await fetch(`http://localhost:3005/api/admin/customization-groups/${testGroupId}`, {
          headers: {
            'Cookie': cookies || ''
          }
        });
        ...
        console.log('\n📡 Testing PATCH group...');
        const patchResponse = await fetch(`http://localhost:3005/api/admin/customization-groups/${testGroupId}`, {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
            'Cookie': cookies || ''
        ...
        const getOptionsResponse = await fetch(`http://localhost:3005/api/admin/customization-options?groupId=${testGroupId}`, {
          headers: {
            'Cookie': cookies || ''
          }
        });
...
testCustomizationGroupOperations();
