...
import { Clock, Printer, Timer, Phone, MapPin, User, Maximize2, Minimize2 } from 'lucide-react';
...
const StoreKitchenDisplay = () => {
  ...
  const getMenuItemCustomizations = (item: OrderItem) => {
    ...
    item.customizations.forEach(customization => {
      ...
      if (!customization?.customizationOption?.group?.name || !customization?.customizationOption?.name) {
        console.warn('Invalid customization data:', customization);
        return;
      }
  ...
  // Fetch orders from store API (no authentication required)
  const fetchOrders = async () => {
    try {
      ...
      // Handle the response format from our store API
  ...
  useEffect(() => {
    // Start fetching orders immediately (no auth needed for store display)
    fetchOrders();
    
    // Set up dynamic polling interval
    const setupPolling = async () => {
      const pollingInterval = await KitchenConfig.getPollingInterval();
      console.log(`🔄 Store Kitchen display polling every ${pollingInterval/1000} seconds`);
      return setInterval(fetchOrders, pollingInterval);
    };
    
    const intervalPromise = setupPolling();
  ...
  const updateOrderStatus = async (orderId: string, newStatus: string) => {
    try {
      const response = await fetch(`/api/store/kitchen/orders/${orderId}`, {
        method: 'PATCH',
        headers: {
      ...
      if (result.success) {
        ...
        // Update the local orders state immediately for better UX
...
================================
  ...
  const toggleFullscreen = async () => {
    try {
      ...
      console.log('Fullscreen not available or denied by user');
    }
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          {Array.isArray(orders) && orders.map(order => (
            ...
            >
                ...
                {/* Status Update Buttons and Print */}
                <div className="flex flex-wrap gap-2">
                  ...
                  {order.status === 'pending' && (
                    <button
                      ...
                      className="flex items-center gap-1 bg-blue-600 hover:bg-blue-700 px-3 py-2 rounded text-sm transition-colors text-white font-medium"
                    >
                      ✓ Confirm Order
                    </button>
                  )}
                  ...
                  {(order.status === 'confirmed' || order.status === 'pending') && (
                    <button
                      ...
                      className="flex items-center gap-1 bg-orange-600 hover:bg-orange-700 px-3 py-2 rounded text-sm transition-colors text-white font-medium"
                    >
                      🍳 Start Preparing
                    </button>
                  )}
                  ...
                  {order.status === 'preparing' && (
                    <button
                      ...
                      className="flex items-center gap-1 bg-green-600 hover:bg-green-700 px-3 py-2 rounded text-sm transition-colors text-white font-medium"
                    >
                      ✅ Mark Ready
                    </button>
                  )}
                  ...
                  {order.status === 'ready' && (
                    <button
                      ...
                      className="flex items-center gap-1 bg-purple-600 hover:bg-purple-700 px-3 py-2 rounded text-sm transition-colors text-white font-medium"
                    >
                      🎉 Complete Order
                    </button>
                  )}
...
export default StoreKitchenDisplay;
