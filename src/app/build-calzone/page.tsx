...
function generatePizzaDescription(selection: Selection, detailedToppings: any[], specialtyPizza?: SpecialtyPizza | null): string {
  if (specialtyPizza) {
    ...
    if (modifications.length > 0) {
      return `${specialtyPizza.name} calzone (${modifications.join(', ')})`;
    }
    return `${specialtyPizza.name} calzone`;
  }
  ...
  if (toppings.length === 0) {
    parts.push('cheese calzone');
  } else if (toppings.length <= 3) {
    const toppingNames = toppings.map(t => t.name.toLowerCase());
    parts.push(`${toppingNames.join(' & ')} calzone`);
  } else {
    parts.push(`calzone with ${toppings.length} toppings`);
  }
...
export default function CalzoneBuilder() {
  ...
  useEffect(() => {
    console.log('[CalzoneBuilder] Selection changed:', {
  ...
  useEffect(() => {
    console.log('[CalzoneBuilder] Component mounting with specialtyId:', specialtyId, 'selectedSizeId:', selectedSizeId);
    console.log('[CalzoneBuilder] About to fetch calzone data...');
    fetchPizzaData();
    
    // Load specialty pizza if specified
    if (specialtyId) {
      console.log('[CalzoneBuilder] SpecialtyId found, fetching specialty calzone...');
      fetchSpecialtyPizza(specialtyId);
    } else {
      console.log('[CalzoneBuilder] No specialtyId, this is fresh calzone building');
    }
  }, [specialtyId]);

  const fetchPizzaData = async () => {
    console.log('[CalzoneBuilder] fetchPizzaData started for CALZONE');
    try {
      // Force fresh data - no caching during debugging
      const response = await fetch(`/api/pizza-data?t=${Date.now()}&productType=calzone`, {
        // Force no cache
        cache: 'no-store',
        headers: {
      ...
      if (data.sizes && data.crusts && data.sauces && data.toppings) {
        console.log('[CalzoneBuilder] Setting calzone data...');
        setData(data);
        ...
        // Set default selections ONLY for fresh calzone building
        if (!specialtyId) {
          console.log('[CalzoneBuilder] Fresh calzone - setting defaults');
          // Fresh calzone building - use proper defaults
          ...
          console.log('[CalzoneBuilder] Specialty calzone - NOT setting defaults, waiting for specialty data');
        }
        // For specialty calzones, DON'T set any defaults here - wait for specialty data to load
      } else {
        console.error('[CalzoneBuilder] Invalid calzone data structure:', data);
      }
    } catch (error) {
      console.error('[CalzoneBuilder] Error fetching calzone data:', error);
    } finally {
      console.log('[CalzoneBuilder] fetchPizzaData finished, setting loading to false');
      setLoading(false);
    }
  };

  const fetchSpecialtyPizza = async (specialtyId: string) => {
    console.log('[CalzoneBuilder] fetchSpecialtyPizza started for:', specialtyId);
    try {
      const response = await fetch(`/api/specialty-calzones/${specialtyId}`);
      console.log('[CalzoneBuilder] Specialty calzone response status:', response.status);
      if (response.ok) {
        const specialty = await response.json();
        console.log('[CalzoneBuilder] Specialty calzone data received:', specialty.name);
        ...
        console.error('[CalzoneBuilder] Failed to fetch specialty calzone:', response.status, response.statusText);
      }
    } catch (error) {
      console.error('[CalzoneBuilder] Error fetching specialty calzone:', error);
    }
  };

  const loadSpecialtyPizzaSelection = (specialty: SpecialtyPizza) => {
    console.log('[CalzoneBuilder] loadSpecialtyPizzaSelection called with:', specialty.name);
    if (!data) {
      console.log('[CalzoneBuilder] No data available, returning');
      return;
    }
    
    console.log('[CalzoneBuilder] Data available, loading specialty selection...');
    console.log('[CalzoneBuilder] Specialty object:', specialty);
    
    // Find the specialty calzone components in the data
    ...
    console.log('[CalzoneBuilder] Selected specialty size:', specialtySize?.name);
    console.log('[CalzoneBuilder] Selected specialty crust:', specialtyCrust?.name);
    console.log('[CalzoneBuilder] Selected specialty sauce:', specialtySauce?.name);
    ...
    console.log('[CalzoneBuilder] Toppings source:', toppingsSource);
    ...
    console.log('[CalzoneBuilder] Specialty toppings:', specialtyToppings.length, 'items');
    ...
    console.log('[CalzoneBuilder] Setting new selection:', newSelection);
    
    // Update selection with specialty calzone data
  ...
  const handlePlaceOrder = async () => {
    if (!selection.size || !selection.crust || !selection.sauce || !data) {
      showToast('Please complete your calzone selection', { type: 'warning' });
      return;
    }
  ...
  if (!data) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          <p className="text-red-600">Failed to load calzone data. Please refresh the page.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-amber-600 text-white py-4">
        <div className="max-w-7xl mx-auto px-4 flex items-center justify-between">
          <div className="flex items-center">
            ...
            {specialtyPizza ? (
              <p className="ml-2 text-amber-200 text-sm">{specialtyPizza.description}</p>
            ) : (
              <p className="ml-2 text-amber-200">
          ...
          <div className="flex items-center gap-4">
            <div className="text-right">
              ...
              <div className="text-amber-200 text-sm">Current Calzone</div>
            </div>
            {cartItems.length > 0 && (
              <>
                <div className="text-right">
                ...
                <button
                  onClick={() => {
                    ...
                    addDetailedPizza({
                      ...
                      notes: notes || (specialtyPizza ? `Specialty Calzone: ${specialtyPizza.name}` : ''),
                      totalPrice: calculateTotal(),
                      specialtyPizzaName: specialtyPizza?.name
                    });
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* Left Side - Calzone Visualization */}
          <div className="bg-white rounded-lg shadow-lg p-8">
            <h2 className="text-xl font-semibold text-center mb-6">
              {specialtyPizza ? 
                `Customizing: ${specialtyPizza.name} Calzone` : 
                'Your Calzone Preview'
              }
            </h2>
            ...
            <div className="flex justify-center mb-8">
              <div className="relative">
                {specialtyPizza && specialtyPizza.imageUrl ? (
                  /* Specialty Calzone Image */
                  <div className="relative w-64 h-64 rounded-full overflow-hidden border-4 border-amber-400 shadow-xl">
                    <img 
                    ...
                    />
                    {/* Fallback calzone visualization */}
                    <div className="hidden absolute inset-0">
                      {/* Outer glow */}
                      <div className="absolute inset-0 rounded-full blur-lg opacity-50 scale-110 bg-amber-200"></div>
                      {/* Calzone base */}
                      <div className="relative w-full h-full rounded-full bg-gradient-to-br from-amber-300 via-yellow-300 to-orange-300">
                        {/* Product icon in center */}
                        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
                          <div className="text-6xl opacity-60 text-amber-700">🥟</div>
                        </div>
                      </div>
                    </div>
                  </div>
                ) : (
                  /* Default Calzone Visualization */
                  <div className="relative">
                    {/* Outer glow */}
                    <div className="absolute inset-0 rounded-full blur-lg opacity-50 scale-110 bg-amber-200"></div>
                    {/* Calzone base */}
                    <div className="relative w-64 h-64 rounded-full border-4 shadow-xl bg-gradient-to-br from-amber-300 via-yellow-300 to-orange-300 border-amber-400">
              ...
              <div className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-lg p-4 mb-6">
                <div className="text-center">
                  ...
                  <div className="text-xs text-gray-600">
                    Base Price ({selection.size?.name || 'Default'}): <span className="font-semibold text-amber-600">
                      ${(() => {
                        if (selection.size) {
                          const sizeOption = specialtyPizza.availableSizes?.find(s => s.id === selection.size!.id);
            ...
            {/* Calzone Details Summary */}
            <div className="space-y-3 text-sm">
              <div className="flex justify-between">
                <span className="font-medium">Size:</span>
                <span className="text-amber-600">{selection.size?.name || 'Not selected'}</span>
              </div>
              <div className="flex justify-between">
                <span className="font-medium">Sauce:</span>
                <span className="text-amber-600">{selection.sauce?.name || 'Not selected'}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-amber-600">{selection.toppings.length}</span>
              </div>
              <hr className="my-3" />
              <div className="flex justify-between text-lg font-bold">
                <span>Total:</span>
                <span className="text-amber-600">${(calculateTotal() || 0).toFixed(2)}</span>
              </div>
            </div>
            ...
            <button
              ...
              disabled={!selection.size || !selection.crust || !selection.sauce}
              className="w-full mt-6 bg-amber-600 text-white py-3 px-6 rounded-lg font-semibold hover:bg-amber-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-all flex items-center justify-center"
            >
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {specialtyPizza?.availableSizes ? (
                      // For specialty calzones, use available sizes with pricing from the specialty calzone
                      specialtyPizza.availableSizes.map((size) => (
                        ...
                        >
                          <div className="flex justify-between items-center">
                            ...
                            <div className="text-right">
                              <div className="font-bold text-amber-600">${size.price.toFixed(2)}</div>
                              {selection.size?.id === size.id && (
                      ...
                      // For custom calzones, use regular pizza sizes
                      data.sizes.map((size) => (
                        <button
                          key={size.id}
                        ...
                        >
                          <div className="flex justify-between items-center">
                            ...
                            <div className="text-right">
                              <div className="font-bold text-amber-600">${(size.basePrice || 0).toFixed(2)}</div>
                              {selection.size?.id === size.id && (
                                <div className="text-xs text-green-600 font-medium">Selected</div>
                              )}
                            </div>
              ...
              {activeTab === 'TOPPINGS' && (
                <div>
                  ...
                  {groupedToppings[activeToppingCategory] && (
                    <div className="mb-6">
                      {/* Specialty Calzone Changes Summary */}
                      {specialtyPizza && (
                                      ...
                                      className={`flex-1 py-2 px-3 rounded-lg text-sm font-medium transition-all ${
                                        isSelected
                                          ? wasOriginallyOnSection
                                            ...
                                            : 'bg-amber-600 text-white shadow-md'   // New topping, selected
                                          : wasOriginallyOnSection
                                            ? 'bg-red-100 text-red-700 border-2 border-red-300 hover:bg-red-200' // Original topping, removed
                                            : 'bg-gray-100 text-gray-700 hover:bg-amber-100 hover:text-amber-700' // Normal unselected
                                      }`}
                                    >
                                      {section}
