...
export async function POST() {
  try {
    ...
    for (const item of createdItems) {
      // All dinner plates get fries/rings, side salad, and condiments
    ...
    return NextResponse.json({
      success: true,
      message: 'Dinner Plates category created successfully',
      data: {
        category: dinnerPlatesCategory,
        itemsCreated: createdItems.length,
        customizationGroups: 5
      }
    });
    ...
    return NextResponse.json(
      { 
        success: false, 
        error: 'Failed to create dinner plates category',
