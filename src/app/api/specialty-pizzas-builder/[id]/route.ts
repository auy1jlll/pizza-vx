...
export async function GET(
  ...
  try {
    ...
    console.log(`📥 Fetching specialty pizza for builder with ID: ${id}`);
    ...
    if (!specialtyPizza) {
      console.log(`❌ Specialty pizza not found: ${id}`);
    ...
    console.log(`✅ Successfully fetched specialty pizza for builder: ${transformedPizza.name}`);
    return NextResponse.json(transformedPizza);
  } catch (error) {
    console.error('Error fetching specialty pizza for builder:', error);
    return NextResponse.json(
