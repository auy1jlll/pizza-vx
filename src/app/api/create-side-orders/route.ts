...
export async function POST(request: NextRequest) {
  try {
    ...
    const sauceOptions = [
      ...
      { name: 'Marinara Sauce', priceModifier: 0 },
      { name: 'Ranch Dressing', priceModifier: 0 },
      { name: 'Duck Sauce', priceModifier: 0 },
      { name: 'Oil & Vinegar', priceModifier: 0 },
      { name: 'Extra BBQ Sauce', priceModifier: 0.50 },
      { name: 'Extra Blue Cheese', priceModifier: 0.50 },
      { name: 'Extra Buffalo Sauce', priceModifier: 0.50 },
      { name: 'Extra Marinara Sauce', priceModifier: 0.50 },
      { name: 'Extra Ranch Dressing', priceModifier: 0.50 }
    ];
    ...
    for (let i = 0; i < sauceOptions.length; i++) {
      ...
      await prisma.customizationOption.create({
        data: {
          name: sauce.name,
          description: sauce.priceModifier === 0 ? 'Free with order' : 'Additional charge',
    ...
    return NextResponse.json({
      success: true,
      message: 'Side Orders category created successfully!',
      itemCount: sideOrderItems.length
    });
