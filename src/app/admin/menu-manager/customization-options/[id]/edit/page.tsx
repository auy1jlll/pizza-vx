...
export default function EditCustomizationOptionPage() {
  ...
  useEffect(() => {
    if (params.id) {
      fetchOption(params.id as string);
      fetchGroups();
    }
  }, [params.id]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert('Error updating customization option: ' + (error.error || 'Unknown error'));
      }
    } catch (error) {
      console.error('Error updating customization option:', error);
      alert('Error updating customization option');
    } finally {
      setSaving(false);
    }
  };
    ...
    <AdminLayout>
      <div className="max-w-2xl mx-auto space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Edit Customization Option</h1>
            <p className="text-gray-600">Update customization option information</p>
          </div>
        </div>
