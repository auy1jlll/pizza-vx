  ...
  async createOrder(data: OrderCreationData) {
    ...
    try {
      return await this.withTransaction(async (tx) => {
        ...
        try {
          console.log('💰 Creating pricing snapshots...');
          await this.createPricingSnapshots(tx, order.id, data.items);
          console.log('✅ Pricing snapshots created successfully');
        } catch (snapshotError) {
          console.warn('⚠️ Pricing snapshots creation failed, but order will still be created:', snapshotError);
          // Don't let snapshot errors prevent order creation
        }

        console.log('🎉 Order creation complete, returning order:', { id: order.id, orderNumber: order.orderNumber });
        return order;
      });
    } catch (error) {
