...
async function testJWTImprovements() {
  ...
  try {
    ...
    if (loginResponse.ok) {
      ...
      if (refreshTokenMatch) {
        // Try refresh
        const refreshResponse = await fetch('http://localhost:3003/api/auth/refresh', {
          method: 'POST',
          headers: {
  ...
  try {
    ...
    if (loginResponse.ok) {
      ...
      // Logout
      const logoutResponse = await fetch('http://localhost:3003/api/auth/logout', {
        method: 'POST',
        headers: {
          'Cookie': cookies || ''
        }
  ...
  try {
    ...
    if (loginResponse.ok) {
      ...
      // Get sessions
      const sessionsResponse = await fetch('http://localhost:3003/api/auth/sessions', {
        method: 'GET',
        headers: {
          'Cookie': cookies || ''
        }
  ...
  try {
    const refreshTokens = await prisma.$queryRaw`
  ...
  try {
    const blacklistCount = await prisma.$queryRaw`
  ...
  try {
    const secretsCount = await prisma.$queryRaw`
